...
const PlanCard = styled(motion.div)<{ featured?: boolean }>`
  ...
  border: 1px solid
    ${({ featured }) =>
      featured ? theme.colors.primary + "60" : theme.colors.glassBorder};
    ...
    <Section>
      <Container>
        <SectionHeader>
          ...
          <SectionTitle>
            {t("coveragePlans.title")}{" "}
            <span>{t("coveragePlans.titleHighlight")}</span>
          </SectionTitle>
          <SectionDescription>
        ...
        <PlansGrid>
          {plans.map((plan, index) => (
            ...
            >
              {plan.featured && (
                <FeaturedBadge>{t("coveragePlans.mostPopular")}</FeaturedBadge>
              )}
